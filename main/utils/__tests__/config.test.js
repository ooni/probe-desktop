...
describe('Migration tests', () => {
  test('Migrate 0->5', async () => {
    ...
    await fs.writeJson('test/mockFiles/config.json', draftConfig, {
      spaces: '  ',
    })
    const initialConfig = await fs.readJSON('test/mockFiles/config.json')
    expect(initialConfig['_version']).toBe(0)

    await maybeMigrate()

    const finalConfig = await fs.readJSON('test/mockFiles/config.json')
